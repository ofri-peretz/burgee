...
| id | Requirement |
| :-- | :-- |
| W1 | `interlace dev <entry>` watches the entry's module graph and reloads on change, with a fresh `Runtime` each time |
...
sequenceDiagram
  ...
  Dev->>Watch: interlace dev ./cli.ts
...
- **Making `interlace dev` the documented entry point for new projects.** It is a
