...
| **3 · surfaces** | `cli-mcp`, `commander-schema`, `commander-env`, `commander-completions` | `--schema`, `--mcp`, completions — the reason to switch | 🔨 `--schema` and `--mcp` served from the manifest on every program, commander syntax included (N1–N6, N8, N9); completions, env and the schema DSL next |
