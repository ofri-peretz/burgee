...
| 21 | [`yargs-compat/`](./yargs-compat/) | `burgee/yargs` — yargs 18 and its whole dependency tree ported method for method, graded by yargs' 804 tests; 782 pass (real yargs: 783 — the one left asserts identity with the yargs-parser npm package) | X1–X8 | review |
...
compat-yargs        ███████████████████████░   782 /  804   97.3%     ███████████████████████░   783 /  804   97.4%
...
| **4 · reach** | `yargs-compat`, `dev-loop`, `cli-modularity`, `cli-prompts`, `first-adopter`, `eslint-plugin-cli-floor`, `docs-deploy`, `cli-benchmarks`, `brand-burgee` | the second host, the dev loop, a CLI we did not write, one brand declaration | 🔨 `burgee/yargs` 782/804 on the first run (= real yargs − 1), byte-identical on the demo (X7, 26 cases), locales shipped; next: burgee's additions on yargs syntax, then `dev-loop` |
...
| `compat-yargs-pass-rate` | yargs' 804 tests | 782 / 804 (2026-09-08), ratcheting |
