...
| id | Requirement |
| :-- | :-- |
| X1 | `burgee/commander` exposes commander's public surface (151 methods) over `cli-core` |
...
`COMPAT_TARGET=burgee/commander npm run compat` — the loop, exiting non-zero below baseline.
