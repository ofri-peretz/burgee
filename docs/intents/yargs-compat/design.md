...
| id | Requirement |
| :-- | :-- |
| X1 | `selvage/yargs` exposes yargs's public surface (108 methods) over `cli-core` |
...
`COMPAT_TARGET=selvage/yargs npm run compat` — the loop, exiting non-zero below baseline.
