...
number appears in CI on every PR, in the docs site, and in `.agent/control-bands.json`.

## Why now

Measured 2026-09-06 (`docs/research/competitor-landscape.md` §5). commander's 105 test
...
replacement (`docs/research/competitor-landscape.md` §5) is only credible if its claim
...
- `.github/workflows/quality.yml` gains a `compat` job; `quality-full.yml` gains the
  Node × OS matrix.
- `.agent/control-bands.json` gains `compat-commander-pass-rate`,
...
2. Baselines are recorded in `.agent/` and only a PR can lower one, with a written
