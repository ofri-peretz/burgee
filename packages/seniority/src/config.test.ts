...
describe("discovery order (V6)", () => {
  it("lists explicit env, the cwd files by extension, then the user directory", () => {
    ...
    const list = candidates({
      name: "my-tool",
      cwd: w,
      env: { MY_TOOL_CONFIG: "x.json", XDG_CONFIG_HOME: xdg },
    });
    expect(list.map((c) => c.path)).toEqual([
      join(w, "x.json"),
      join(w, "my-tool.config.json"),
      join(w, "my-tool.config.mjs"),
      join(w, "my-tool.config.js"),
      join(w, "my-tool.config.cjs"),
      join(xdg, "my-tool", "config.json"),
    ]);
  ...
  it("finds ./name.config.json in the cwd and nothing when there is none (silent)", async () => {
    ...
    expect(
      await discover({ name: "app", cwd: join(dir, "empty"), env: {} }),
    ).toBeUndefined();
  });
  ...
  it("loads a JavaScript config, object or function", async () => {
    ...
    expect((await discover({ name: "app", cwd, env: {} }))?.data).toEqual({
      region: "fn",
    });
  });
  ...
  it("--config names a file explicitly; a missing one is a CONFIG error, not silence (yargs #1676)", async () => {
    ...
    expect(
      (await discover({ name: "app", cwd: dir, env: {}, explicit }))?.data,
    ).toEqual({ region: "x" });
    await expect(
      discover({
        name: "app",
  ...
  it("--no-config disables discovery entirely, explicit env included", async () => {
    ...
    expect(
      await discover({
        name: "app",
        cwd: join(dir, "n"),
        env: { APP_CONFIG: join(dir, "n/app.config.json") },
        disabled: true,
      }),
    ).toBeUndefined();
  });
  ...
  it("reports invalid JSON as a CONFIG error naming the file", async () => {
    ...
    await expect(
      discover({ name: "app", cwd: join(dir, "bad"), env: {} }),
...
describe("extends (V7)", () => {
  ...
  it("resolves relative to the extending file, left to right, and records the chain", async () => {
    ...
    const top = write("x/app.config.json", {
      extends: ["./team.json"],
      region: "top",
    });
    ...
    expect(loaded.chain.map((p) => basename(p))).toEqual([
      "base.json",
      "team.json",
      "app.config.json",
    ]);
