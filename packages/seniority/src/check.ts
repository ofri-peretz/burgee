/**
 * `seniority check <plugin-file>` — load a plugin, validate it, register it, and show what seniority
 ...
 *     registers into every host in the family — which means a misspelled key is silent. `0 sources`
...
import { PluginError, type PluginErrorCode, register, reset, validate } from './plugin.js';

/**
 * E1, restated rather than imported: seniority is an independent product and depends on nobody in
...
async function inspect(argv: readonly string[], write: (s: string) => void): Promise<number> {
  ...
  const rows = Object.keys((plugin as { sources?: Record<string, unknown> }).sources ?? {}).map((source) => `${source}`);
  write(`${name} — ${String(rows.length)} sources\n`);
  if (rows.length === 0) {
    return refuse(
      'E_NO_CONTRIBUTION',
      `${name} registers, but contributes nothing seniority reads`,
      'add a `sources` section — a key another package in the family reads is allowed in the same object, but `seniority check` cannot show it',
      write,
    );
  }
