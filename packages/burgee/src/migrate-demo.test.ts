...
describe('demo-cli-commander — the codemod against the hand-written drop-in', () => {
  ...
  it('reports the real counts, refuses nothing, and exits OK', async () => {
    ...
    expect(r.dependencies).toEqual({ before: ['commander'], removable: ['commander'], after: 0 });
    expect(r.graded).toEqual([{ host: 'commander', reference: 1360, passed: 1360, rate: 1 }]);
  });
});
...
describe('demo-cli-yargs — the second case', () => {
  ...
  it('reports the real counts and the oracle’s yargs row', async () => {
    ...
    expect(r.graded).toEqual([{ host: 'yargs', reference: 804, passed: 804, rate: 1 }]);
    expect(r.dependencies).toEqual({ before: ['yargs'], removable: ['yargs'], after: 0 });
  });
});
