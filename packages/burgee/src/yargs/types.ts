/**
 * yargs' public **types**, for `import yargs, { type Argv, … } from 'burgee/yargs'`.
 *
 * yargs ships no declarations; every typed yargs program reads them from `@types/yargs`.
 * This is that package's surface, ported from `@types/yargs` 17.0.35 (with
 * `Configuration` from `@types/yargs-parser` 21.0.3) — DefinitelyTyped, MIT — and
 * exported under the same names the ESM entry `index.d.mts` exports, so a program's
 * `import type` lines move with its value import and nothing else changes. Declarations
 * only: this module emits no code (J9 — no dependency on either package).
 *
 * Four departures, each deliberate, each wider than or equal to what it replaces:
 *
 * - **`Argv` is not callable.** @types/yargs gives the instance a call signature from
 *   yargs 16's singleton; a yargs 18 instance is not a function, and neither is burgee's.
 * - **`onFinishCommand` is gone.** yargs 17 removed it and @types/yargs still declares it;
 *   a method the type promises and the instance lacks is a TypeError that compiles.
 *   `facade-types.test.ts` checks every member of `Argv` against a live instance.
 * - **A command handler may return a value** (`=> unknown`, not `=> void | Promise<void>`):
 *   under `--json` burgee's envelope carries it. Every handler @types/yargs accepts still fits.
 * - **burgee's additions are declared** — `manifest`, `effects()`, `use()`, `burgee()` —
 *   so they survive a typed chain.
 *
 * The internal helpers (`CamelCaseKey`, `IsUnknown`, …) are declared and not exported,
 * as in `index.d.mts`.
 */
import type { DeclaredEffects, Manifest, Plugin } from '../manifest.js';
import type { DetailedArguments } from '../yargs-parser.js';

import type { BurgeeSeam } from './factory.js';

/** yargs-parser's configuration keys — `.parserConfiguration()` takes a `Partial` of these. From `@types/yargs-parser` 21.0.3. */
...
export type BuilderCallback<T, R> =
...
export interface Argv<T = {}> {
  ...
  command<U = T>(
    ...
    handler?: (args: ArgumentsCamelCase<U>) => unknown,
  ...
  command<O extends { [key: string]: Options }>(
    ...
    description: string,
    builder?: O,
    handler?: (args: ArgumentsCamelCase<InferredOptionTypes<O>>) => unknown,
  ...
  // ───── burgee: additive, not in @types/yargs — what `burgee/yargs` adds over yargs ─────

  /** burgee: the manifest every surface — help, `--json`, `--schema`, `--mcp`, completions — is projected from (J7, J8). */
  readonly manifest: Manifest;

  /** burgee: declare what the command does to the world (N6); what exposes it as an MCP tool (N2). */
  effects(value: DeclaredEffects): Argv<T>;

  /** burgee: plugins yargs never had. `preRun`/`postRun` fire around every handler. */
  use(plugin: Plugin): Argv<T>;

  /** burgee: inject the streams and the exit for this instance (T1), and get E1 exit codes back. */
  burgee(seam: BurgeeSeam): Argv<T>;
}
...
export type InferredOptionTypePrimitive<O extends Options | PositionalOptions> = O extends { default: infer D }
...
export type InferredOptionTypeInner<O extends Options | PositionalOptions> = O extends
...
export type AsyncCompletionFunction = (
  current: string,
  argv: any,
  done: (completion: readonly string[]) => void,
) => void;
export type PromiseCompletionFunction = (current: string, argv: any) => Promise<string[]>;
export type FallbackCompletionFunction = (
  current: string,
  argv: any,
  completionFilter: (onCompleted?: CompletionCallback) => any,
  done: (completions: string[]) => any,
) => void;
export type MiddlewareFunction<T = {}> = (args: ArgumentsCamelCase<T>) => void | Promise<void>;
export type Choices = ReadonlyArray<string | number | true | undefined>;
export type PositionalOptionsType = 'boolean' | 'number' | 'string';
export type CompletionCallback = (err: Error | null, completions: string[] | undefined) => void;

export type BuilderArguments<T extends (...args: any) => R, R = ReturnType<T>> = R extends Argv<infer X>
