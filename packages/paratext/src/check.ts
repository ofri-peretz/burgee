/**
 * `paratext check <plugin-file>` — load a plugin, validate it, register it, and show what paratext
 ...
 *     registers into every host in the family — which means a misspelled key is silent. `0 capabilities`
...
/**
 * E1, restated rather than imported: paratext is an independent product and depends on nobody in
...
const USAGE = 'usage: paratext check <plugin-file>\n';
const HELP = `${USAGE}
Load a plugin file, validate it against the family schema, register it, and report what
paratext does with it. Exit 0 when it contributes, 1 on a refusal (with a code and a fix), 2 on
a usage error.
 ...
 * before this, `paratext --help` reached the `import()` and failed as `Cannot find module '…/--help'`
...
async function inspect(argv: readonly string[], write: (s: string) => void): Promise<number> {
  ...
  const rows = contributions()
    ...
    // Both projections, because that is the point of a capability: the bytes for a terminal that
    // understands, and what it prints for one that does not. An author who only ever sees the
    // first ships the second broken.
    .map((c) => `${c.name}  osc ${String(c.capability.osc)}  encode ${JSON.stringify(c.capability.encode)}  fallback ${JSON.stringify(c.capability.fallback)}${shadows(c.shadowed)}`);
  write(`${name} — ${String(rows.length)} capabilities\n`);
  if (rows.length === 0) {
    return refuse(
      'E_NO_CONTRIBUTION',
      `${name} registers, but contributes nothing paratext reads`,
      'add a `capabilities` section — a key another package in the family reads is allowed in the same object, but `paratext check` cannot show it',
      write,
    );
  }
