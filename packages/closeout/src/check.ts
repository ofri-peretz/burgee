/**
 * `closeout check <plugin-file>` — load a plugin, validate it, register it, and show what closeout
 ...
 *     registers into every host in the family — which means a misspelled key is silent. `0 handlers`
...
/**
 * E1, restated rather than imported: closeout is an independent product and depends on nobody in
...
const USAGE = 'usage: closeout check <plugin-file>\n';
const HELP = `${USAGE}
Load a plugin file, validate it against the family schema, register it, and report what
closeout does with it. Exit 0 when it contributes, 1 on a refusal (with a code and a fix), 2 on
a usage error.
 ...
 * before this, `closeout --help` reached the `import()` and failed as `Cannot find module '…/--help'`
...
async function inspect(argv: readonly string[], write: (s: string) => void): Promise<number> {
  ...
  const rows = contributions()
    ...
    .map((c) => `${c.id}  runs in the ${c.phase} phase`);
  write(`${name} — ${String(rows.length)} handlers\n`);
  if (rows.length === 0) {
    return refuse(
      'E_NO_CONTRIBUTION',
      `${name} registers, but contributes nothing closeout reads`,
      'add a `handlers` section — a key another package in the family reads is allowed in the same object, but `closeout check` cannot show it',
      write,
    );
  }
