/**
 * `linegauge check <plugin-file>` — load a plugin, validate it, register it, and show what linegauge
...
import { overrides, PluginError, type PluginErrorCode, register, setOverrides, validate } from './plugin.js';
...
const USAGE = 'usage: linegauge check <plugin-file>\n';
const HELP = `${USAGE}
Load a plugin file, validate it against the family schema, register it, and report what
linegauge does with it. Exit 0 when it contributes, 1 on a refusal (with a code and a fix), 2 on
a usage error.
 ...
 * before this, `linegauge --help` reached the `import()` and failed as `Cannot find module '…/--help'`
...
/** A code point as Unicode writes it: `U+` and at least four uppercase hex digits. */
const HEX = 16;
const MIN_DIGITS = 4;
const codePoint = (n: number): string => `U+${n.toString(HEX).toUpperCase().padStart(MIN_DIGITS, '0')}`;
...
/** The overrides a plugin declares, read without trusting it past what `validate` has proved. */
const overridesOf = (plugin: unknown): [string, Override][] => {
  const widths = (plugin as { widths?: unknown } | null)?.widths;
  return typeof widths === 'object' && widths !== null ? Object.entries(widths as Record<string, Override>) : [];
};
...
async function inspect(argv: readonly string[], write: (s: string) => void): Promise<number> {
  ...
  if (rows.length === 0) {
    return refuse(
      'E_NO_CONTRIBUTION',
      `${name} registers, but contributes nothing linegauge reads`,
      'add a `widths` section — a key another package in the family reads is allowed in the same object, but `linegauge check` cannot show it',
      write,
    );
  }
