...
const USAGE = 'usage: bellpull check <plugin-file>\n';
const HELP = `${USAGE}
Load a plugin file, validate it against the family schema, register it, and report what
bellpull does with it. Exit 0 when it contributes, 1 on a refusal (with a code and a fix), 2 on
a usage error.
 ...
 * before this, `bellpull --help` reached the `import()` and failed as `Cannot find module '…/--help'`
