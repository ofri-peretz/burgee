...
const ALLOWED: Record<string, string[]> = {
  ...
  'ora.js': ['./cursor.js', './runtime.js', './spinners.json'],
  ...
  'log-update.js': ['./cursor.js', './runtime.js'],
...
const INTERNAL_ALLOWED: Record<string, string[]> = {
  ...
  // The two modules Y9 added an edge to and from. `cursor.js` installs signal listeners on
  // the process, so it is one of the four files that used to name it; `runtime.js` is where
  // that name went, and it is a leaf — the seam reaches nothing, which is the point of it.
  'cursor.js': ['./runtime.js'],
  'runtime.js': [],
};
