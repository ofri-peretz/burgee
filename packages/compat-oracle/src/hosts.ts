...
export interface Host {
  ...
  /**
   * How its suite is executed. `ava` runs through `node:test` with the `ava` shim; `vitest`
   * is what a jest suite runs under, since jest's globals are vitest's and vitest is
   * already here.
   */
