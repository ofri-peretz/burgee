...
export const HOSTS: Host[] = [
  ...
  },
  {
    ...
    status: 'active',
    note: "15 / 15 control, 14 / 15 target, measured 2026-09-15 — up from 13 / 15, and **14 / 15 is this row's ceiling**. The remaining failure is named rather than excluded, because an ava host's TAP reaches `summarize` through the summary-line dialect and the oracle refuses an exclusion it cannot match by name — which is the right refusal here. (1) `can slice a string with unknown ANSI color` **was** a real gap and is now closed: slice-ansi re-emits *any* SGR parameter it saw and closes with a reset, so `ESC[1001m` survives a cut, while `linegauge`'s style stack tracked only the codes in its own close-code table and dropped the rest, returning a bare `TES`. Ours was the wrong answer — the sequence is the caller's, not the library's to vet, and a stack that discards what it cannot name fails in the worst direction: the text survives and its style does not, silently. `style.ts` now carries an unrecognised parameter through as its own family and closes it with `ESC[0m`, which is the only closer correct for a parameter whose meaning is unknown. (2) `slice links` is `test.failing()` in slice-ansi's *own* suite: the incumbent cannot round-trip an `OSC 8` hyperlink and says so. `linegauge` can, and ava reports a passing `test.failing` as `not ok`. So the one failure left on this row is the target being **more** correct than the host, which is exactly the sort of number a compat rate must not quietly launder — 14 / 15 stands, with the reason beside it, and no edit to this package can make it 15. Its suite imports `random-item`, committed under `vendor/slice-ansi/node_modules/`.",
  },
  {
