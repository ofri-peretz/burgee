...
string-width's options carry over under their own names and defaults. Ambiguous-width
characters count narrow unless you pass `ambiguousIsNarrow: false`, which counts them two
columns wide for a terminal rendering with a CJK font; `countAnsiEscapeCodes: true` measures
escape sequences instead of removing them.
