...
```json
{ "overrides": { "exit-hook": "npm:closeout@^0.4", "restore-cursor": "npm:closeout@^0.4" } }
```
...
Weight, installed and tree-inclusive: **102,288 bytes** against **170,604** for the incumbents it replaces — a ratio of **0.5996** (exit-hook measured but left out of the ceiling, so it is understated).
## Where it sits
