...
import { packageDocsUrl } from './config';
...
export function publicPackages(dir: string = PACKAGES_DIR): PublicPackage[] {
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    found.push({ name: manifest.name, description, replaces: replacesOf(manifest.name, description), url: packageDocsUrl(manifest.name) });
  }
