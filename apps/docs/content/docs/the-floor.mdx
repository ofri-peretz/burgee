---
title: The floor
description: The requirements every CLI on burgee meets, and who holds each — runtime, lint, or both. 74 of the floor's 114 are tabled here.
---

The floor is **114 requirements**: the 109 rows of the requirement tables in
[`.sdlc/intents/burgee/spec.md`](https://github.com/ofri-peretz/burgee/blob/main/.sdlc/intents/burgee/spec.md#requirements--the-cli-floor)
plus the five shape-lock rows Z1–Z5 above them, which is the count that spec states. This
page tables **74** of them — the families below. The spec is the complete list, and what is
built, requirement by requirement, is recorded there too.

Each requirement is held by the **runtime**, by the **lint** plugin
...
[Research](https://github.com/ofri-peretz/burgee/tree/main/.sdlc/research).
...
| ID | Requirement | Holds |
| :-- | :-- | :-- |
| K1 | No dependency outside the burgee family in any published package; hosts and UI libraries are peers | lock |
| K2 | ESM only, Node 20.19+ or 22.13+ (restated 2026-09-23, D-132; was Node 24 or newer) | lock |
...
| N2 | A command is kept from agents only by a declaration: `effects: 'withheld'`. Every runnable command on burgee's own API declares its effects; a façade command that declared nothing is listed as `effects: 'undeclared'`, with no hints | both |
| N3 | No dependency outside the burgee family: JSON-RPC over stdio against `node:readline` | lock |
