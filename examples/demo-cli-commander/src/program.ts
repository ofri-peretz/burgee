/**
 * The reference CLI, written once in commander's syntax. `index.ts` runs it on real
 * commander, `burgee.ts` on `burgee/commander` — the same source, one import swapped,
 * which is exactly the migration a user makes. The conformance suite diffs the two
 * byte for byte (commander-compat X7).
 *
 * Handlers write through the `Runtime` they are built against, never `console`, so the
...
/** The two classes the demo needs, from whichever library provides them. */
...
export function buildProgram(lib: CommanderLike, rt: Runtime): Command {
  const program = new lib.Command('demo').description('The Interlace CLI reference demo').version('0.0.0');
  ...
  program
    ...
    .addOption(new lib.Option('--greeting <word>', 'the greeting word').env('DEMO_GREETING').default('Hello'))
