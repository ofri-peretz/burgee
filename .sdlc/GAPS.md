...
| A11 | paratext: ansi-escapes **4/4** (D-138), terminal-link 8/10, term-img 12/18 — **each at a recorded ceiling**: CSI is out of scope (3 of ansi-escapes' 4), terminal-link's 2 read another package's mutated module object, term-img's 6 pass a PATH where D-030 takes bytes. terminal-link's baseline was stale at 0 and is now 8 | compat baselines | each ceiling restated as a decision, or lifted |
| A12 | seniority: cosmiconfig 186/243, dotenv **106**/141 (was 80), lilconfig **77/77** (was 67; all ten were the harness's jest.mock), rc **1/1** (was 0) — D-135 lifted the two; dotenv's remaining 35 are the declined vault/decrypt (27), dotenvx tips (2), and 6 internal-module cases the harness cannot map yet | compat baselines | each suite at 100%, or each failing case named with its reason |
