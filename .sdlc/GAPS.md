...
| ~~A14~~ | ~~the foundation layers are static imports of the engine~~ — **closed by restatement (D-132)**: two in-family specifiers are static and declared, the output stack is dynamic; the `.` weight rule asserts exactly that | burgee U13 | done |
