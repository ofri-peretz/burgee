...
`npx tsx scripts/mine-issues.ts commander-completions` answers `no layer matched`.
...
cites three or more times.** Here that is `commander`, and `yargs`, which it cites five times. Trackers cited once or twice
