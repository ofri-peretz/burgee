...
| C3 | **Not built**, and the workflow says so itself | `compat.yml`'s `matrix` job is `os: [ubuntu-latest, macos-latest, windows-latest] × node: [24]`, above a comment reading *"`engines` still says `>=24`, so C3 is not met while this stands"*. The OS axis is met on all three; the Node axis is one version. Every published package declares `engines.node: ">=24"`, so **Node 26 is claimed and untested**. Putting 26 back in the matrix list is the whole of the remaining work, and nothing else changes | `grep -A4 'node: \[' .github/workflows/compat.yml` |
| C4 | **Not built** | **The page half closed 2026-09-23:** `compatibility.mdx` now renders all four kinds — control-failure allowances, platform-conditional cases and ungraded directories beside the named exclusions — each with its reason, and `scripts/compat-page-subtractions.test.ts` fails when a declared subtraction is missing from the page (ten were, on its first run). What stays open is the sentence as written: an **id** and a test per divergence, which the proposed restatement below drops. No `excluded.json` exists, no divergence carries an **id**, and there is no test asserting any individual divergence — the tests assert the *mechanism*. What exists instead is four kinds of declared subtraction in `src/hosts.ts`, each with a mandatory `why` that a lock refuses empty: `excludes` (cli-table3, lilconfig), `controlFailures` (yargs, cosmiconfig, lilconfig, clack), `ungradedDirs` (yargs) and `conditionalCases` (cosmiconfig, lilconfig). That is stronger than the design asked for in one way — the gate refuses an exclusion that matches nothing (`unmatchedExclusions`) and refuses one on a runner whose TAP has no case names — and **narrower in the way C4 was written to stop**: only `excludes` reaches the published page, so a reader of `compatibility.mdx` sees its 8 named rows and none of the 43 control-failure allowances, the 4 conditional cases or the 3 ungraded directories | `npx vitest run --root packages/compat-oracle -t 'excluding a case that cannot fail for any target'`; then `grep -n 'excludes ??' scripts/compat-page.ts` — one of the four kinds is rendered |
...
| R3 | **Not built** as an absolute, and its mechanism is built | The mechanism holds exactly as designed: the vendor step discovers every internal specifier, `writeInternalShims` writes a shim at the precise path the test reaches for, internals-only files are recorded by name in `.source.json`'s `internalFiles`, and they are graded on a separate informational line (`Grade.internals`) that never enters the gate. **No file is thrown away for importing an internal**, which is what the requirement was for. What makes the flat sentence false is two things added since it was written: `ungradedDirs` copies directories and deliberately does not grade them — yargs' `fixtures`, `helpers` and `esm`, the last a standing decision with a date on it — and `excludes` removes named cases from the gate. Both cost a written reason and both go red when empty or unmatched, but "**No upstream file is excluded**" is no longer a sentence this tree supports | `npx vitest run --root packages/compat-oracle src/vendored-suite.test.ts -t 'every vendored test file is graded or named'` |
...
| R6 | **Not built** | `scripts/vendor-suite.ts` reaches the oracle through **bare specifiers** — `compat-oracle/hosts`, `/provenance`, `/upstream`, `/vendor` — and that export map points at `dist/`. In a worktree that has not been installed, Node's `node_modules` walk climbs out of the worktree and resolves them from the **main checkout's** `packages/compat-oracle/dist/`, while the script's own output paths come from `import.meta.url` and stay here: a vendor run then writes *this* tree's `vendor/` and `PROVENANCE` using *another* tree's `HOSTS` table and `vendor()`. Measured in this worktree on 2026-09-16 — with `node_modules` present the import fails loudly with `ERR_MODULE_NOT_FOUND`; with it absent, `createRequire(<worktree>).resolve('compat-oracle/hosts')` returns `…/burgee/packages/compat-oracle/dist/hosts.js`. `scripts/mine-issues.ts` has the same shape through `compat-oracle/demand`. Nothing checks it, and the failure is silent in exactly the direction that writes a wrong manifest | From a worktree with no `node_modules`: `node -e "console.log(require('node:module').createRequire(process.cwd()+'/x.js').resolve('compat-oracle/hosts'))"` — it prints a path outside the worktree |
| R7 | **Not built** | Three rows are load-sensitive, and it is recorded in prose per host and nowhere in the gate. `exit-hook` kills its fixture on a **fixed 1000 ms clock**: measured at load 22 on 14 cores it scored 21 / 21 six times out of nine and 17 / 21 twice, the same four cases every time, and 0 of 10 fixture runs lost the race when killed on a readiness signal instead. `ansi-escapes` has read 0 / 4 under the same conditions against 4 / 4 at rest. `cross-spawn` timed out its shebang case in 3 of 6 runs at load 16–21 against 68 / 68 at load ~1. One host has a fix and it is per host, not a property of the harness — cosmiconfig's `vitestConfig.testTimeout: 30_000`, after three runs of the *unchanged control* read 240, 238 and 231. There is no repeat-and-agree, no quarantine, and no marker that tells a flake from a regression; `hosts.ts` argues a red row that is loud and occasionally wrong is the right way round, and that is a position, not a measurement | Nothing asserts it. `npx vitest run --root packages/compat-oracle` is unaffected — the hazard is in `npm run compat` under concurrent load, which is the run that publishes |
