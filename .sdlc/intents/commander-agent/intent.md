...
`npx tsx scripts/mine-issues.ts commander-agent` answers `no layer matched`.
...
cites three or more times.** Here that is `commander`, and `yargs`, which it cites four times. Trackers cited once or twice
(citty, clack, oclif) are comparisons this intent draws, not incumbents it replaces; mining them would file
