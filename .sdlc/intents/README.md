...
| 17  | [`caique/`](./caique/)                               | §9           | flags first, errors in non-TTY, `--yes`, `--interactive`       | P1–P3     | approved (D-128) — 1 of 3. The decision table is exhaustive and green. **Not met:** the Ctrl+C raw-mode test uses a synthetic key stream, not a PTY, and runs on one OS, not three                                                                                                                                  |
...
| :-------------------------------------------------------------------------------------------- | :------------------------ | :------------------------------------------------------ |
| the framework                                                                                 | `burgee`                  | ✅ `burgee@0.9.0`                                       |
...
| the colours a CLI carries: policy, tokens, theme, chalk path                                  | `roundel`                 | ✅ `roundel@0.4.0` — chalk path graded 58 / 58          |
| the staff the flag flies from: frame loop, plugin host, ora/boxen/cli-table3/log-update paths | `flagstaff`               | ✅ `flagstaff@0.3.4`                                    |
| the parrot that always answers back: prompts, flags first                                     | `caique`                  | ✅ `caique@0.4.0`                                       |
| measuring and cutting styled text: width, wrap, truncate, slice                               | `linegauge`               | `linegauge@0.4.0` stub; F1                              |
| ending cleanly: exactly-once handlers, a deadline, terminal restore                           | `closeout`                | `closeout@0.3.0` stub; F2                               |
| which source outranks which, and which one set this value                                     | `seniority`               | `seniority@0.4.0` stub; F3                              |
| running another program and getting a result back                                             | `bellpull`                | `bellpull@0.2.0` stub; F4, conditional on its kill gate |
