# `.sdlc/intents/` — Stage 1 (Plan) and Stage 2 (Design)

> **Audited 2026-09-10 against the tree.** Nine rows named a blocker that no longer existed —
> `benchmarks/` "not built" when it carries five axes, `control-bands.yml` "never run" when it
> had run and computed 14 bands, "no control band for the pass rate" in three separate intents
> when six of them compute, `/benchmarks` 404 when the page is generated, `cli-table3 ungraded`
> when it is 29 / 29. Each correction below names the evidence.
>
> A stale blocker is worse than an open one: it is read as work remaining, so the work is
> either redone or deferred behind something already finished. **This file is the roadmap;
> when it is wrong, the plan is wrong.** Nothing here was promoted to `shipped` — the
> corrections remove blockers that are gone, they do not assert criteria that were never met.
>
> **The tenth is the one that stings.** "Give roundel, flagstaff and caique a publish credential"
> was carried as _the oldest open blocker in this file_ — and all three had published at 0.1.0
> on 2026-09-09T18:14Z, with five green `release.yml` runs since. It was quoted as blocking in
> conversation for a day after it was fixed. Checking `npm view roundel version` costs one
> command; not checking it cost a day of a roadmap that said the wrong thing.
...
slogan: _extend with agents_ is U4, U9 and the plugin contract (plugins are data, one
schema, one-turn authoring measured weekly); _beats every incumbent_ is U5 and U11 (lighter
per subpath, graded by their own suites, every number published); _moves faster_ is the
...
**If you are picking up work now**: two of the three bets in _The minimum that proves the
play_ landed on 2026-09-09; the third has not, and it is the only one that needs somebody
...
2. ✅ **roundel ships at 0.1 under picocolors' weight**, with `roundel/chalk` graded by chalk's
   ...
   _"the ceiling is picocolors: 3.3 KB"_, which is a rounded figure and not a measurement of
...
| **Agents do better against a burgee CLI**                  | The floor is the shape the best agent-native CLI already has: Claude Code's own CLI ships a print mode, `--output-format json`, structured errors and never prompts under a pipe; Anthropic's guidance on writing tools for agents (2025) says what O1–O5 and E1–E5 say — actionable errors, structured output, no hangs. Some wins are binary: a hung prompt in non-TTY is a failed task every time; a usage-vs-runtime exit code tells an agent whether to fix or retry. The incumbents' own trackers say the same: clack #533 (prompts in agent environments — "The CLI usually hangs forever and just fails"), #510 (`\r` frames captured verbatim), Inquirer D#1699 (a binary per prompt so a script can drive it), in `output-stack-open-issues.md` §2 | **B1 as a protocol** in `cli-benchmarks`: tasks × callers, same CLI on commander and on burgee, measuring hangs per 100 non-TTY runs, turns to success, success rate; weekly; the honest fallback (parse reliability, not token economy) already recorded                                                                                                       | hangs/100 = 0 by construction; turns and success rate published, hypothesis until they move |
...
family. _Edge_ is what no incumbent can add without changing shape; _proof_ is the number on
the docs site that says it is true; _value_ is who is better off and how we would know.
...
| **native, where measured** | any layer ported to the fastest language and shipped the oxc/esbuild way, graded by the incumbents' own suites | no CLI-tooling incumbent has a prebuild pipeline, a WASM fallback, or a suite-gated way to prove a port changed nothing; adding them is a shape change. Native itself is not the edge — oxc and esbuild did it first — the _suite-graded_ port in _this_ market is | B2/B4 rows for the native build beside the JS build; the same pass rates                                         | authors of large CLIs: manifest and help at hundreds of commands in microseconds; end users: a single binary with no Node install |
...
1. **Measure the layer before naming it.** Every incumbent: weekly downloads, direct
   dependency count, last publish date, declared `engines`, and _real_ installed weight from
...
2. **Apply the platform-absorption test.** Does a Node or npm builtin **substitute** for the
   ...
   RFC 53 (_Native Dependency Patching_, ratified 2026-06-05), and a standalone glob matcher,
...
6. **Grade before recommending, always.** An override edits packages deep in a stranger's
   ...
   The grader is held to rule 8 like everything else, and today it is not:
   [`compat-grader-layer`](./compat-grader-layer/intent.md) says so. `compat-oracle` is
   `"private": true` while `.changeset/config.json` sets `privatePackages: { version: false }`,
   so four authored changesets against it can never be released and the Version PR reopens
   empty on every pass. The empty PR is the symptom; a layer with no name is the cause.

7. **Ship as an override target, not as one more package.** Zero dependencies stops _us_
...
§10 was previously marked _not planned, commander owns it_. Owning a parser (wave 5)
...
| :-- | :----------------------------------------------------- | :----------------------------------------------------------------------------------------- | :-------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | [`sdlc-locks-evals-bands/`](./sdlc-locks-evals-bands/) | intent lock, evals layer 1, control bands                                                  | —         | **review** (was `shipped`) — the locks shipped and are mutation-proven. **The bands ran on 2026-09-09 and computed:** `control-bands.yml` has one successful run, there are 19 bands rather than five, and 14 of them are inside band over 41 observations. Three breached at 1σ, all weight drifting up. Still open: the evals link checker does not read `.sdlc/intents/`, and `agent-tokens-per-task` / `agent-turns-per-task` are fed by B1, which needs a credential                                 |
...
| 3   | [`compat-oracle/`](./compat-oracle/)                   | upstream suites vendored and redirectable; pass rate as a ratcheting gate; the Node matrix | C1–C6     | approved (D-129) — 3 of 6. **Seven** hosts graded with controls, all at 100% (commander 1,360, yargs 804, ora 99, log-update 99, boxen 84, chalk 58, cli-table3 29). **Two of the three gaps closed 2026-09-09/10:** six compat control bands compute, and the root `README.md` publishes the rate. **Still not met:** C3 — the matrix is Node 24 only against `engines: >=24`                                                                                                                                      |
| 4   | [`cli-packaging/`](./cli-packaging/)                   | zero deps, ESM, artifact gate, size ratchet, pay-per-import                                | K1–K6     | review — 2 of 4. Zero deps and the bun/deno smoke hold. **Not met:** ~~nothing checks `files` membership~~ — `scripts/pack-list-lock.test.ts` checks what `exports` promises against the tarball npm actually builds (2026-09-10). `check-published-artifacts.ts` still has no test. ~~the publish job fails `ENEEDAUTH` for three of four packages~~ — **all four publish**: roundel, flagstaff and caique went out at 0.1.0 on 2026-09-09T18:14Z and `release.yml` has five consecutive successful runs |
| 5   | [`cli-benchmarks/`](./cli-benchmarks/)                 | four axes — agent cost, performance, compatibility, weight                                 | B1–B7     | review — **corrected 2026-09-10: the "nothing is built" line was stale.** `benchmarks/` carries five axes (`agent`, `compat`, `perf`, `reliability`, `weight`), `npm run bench` exists, and `/docs/benchmarks` is generated from the published results. B1 (tokens and turns) is still unmeasured and skips loudly without a credential; that is the one axis, not nine                                                                                                                                   |
...
| 31  | [`docs-per-package/`](./docs-per-package/)               | **one docs app per published package**, each on its own `interlace.tools` host — `.github/vercel-apps.json` as the one table both workflows read, a shared private chassis (`apps/docs-chassis`) instead of nine copies, and the family-wide pages (`compatibility`, `comparison`, `gallery`) kept on burgee alone. Nine hosts, one per package (D-131). Extends [`docs-deploy`](./docs-deploy/), which deferred the map "until a second app exists" | B7                         | approved |
...
| 17  | [`caique/`](./caique/)                               | §9           | flags first, errors in non-TTY, `--yes`, `--interactive`       | P1–P3     | approved (D-129) — 1 of 3. The decision table is exhaustive and green. **Not met:** the Ctrl+C raw-mode test uses a synthetic key stream, not a PTY, and runs on one OS, not three                                                                                                                                  |
...
| 20  | [`commander-compat/`](./commander-compat/)     | `burgee/commander` — commander 15 ported method for method, graded by commander's own suite; **1,360 / 1,360 (100%)** of those that run on this OS, level with the real package in the same run | X1–X8            | review — 2 of 4. **The one substantive gap closed on 2026-09-09:** `compat-commander-pass-rate` is a control band and computes, inside band over 41 observations. What remains is the acceptance wording, not a missing control |
| 21  | [`yargs-compat/`](./yargs-compat/)             | `burgee/yargs` — yargs 18 and its whole dependency tree ported method for method, graded by yargs' 804 tests; **804 / 804 (100%)** (real yargs: 802 in the same run)                            | X1–X8            | review — 3 of 4. **Same gap, same close:** `compat-yargs-pass-rate` is a control band and computes, inside band over 41 observations                                                                                            |
...
| 24  | [`roundel/`](./roundel/)                   | **roundel** — `./policy`, `./tokens`, `./theme`, and a chalk path graded by chalk's tests; each subpath at or under the incumbent it replaces | U2 U5 U6 U7 U10 U12 | approved — 3 of 5 criteria met. `roundel/chalk` 58 / 58; the bytes are locked under both ceilings. ~~**Not shipped:** 0.1.0 in the tree, 0.0.1 on npm~~ — **shipped 2026-09-09T18:14Z; `npm view roundel version` is 0.1.0**, and `release.yml`'s last five runs all succeeded. ~~`/benchmarks` does not exist~~ (it does). The two remaining criteria are the acceptance bar, not a credential                                                                                               |
| 25  | [`flagstaff/`](./flagstaff/)               | **flagstaff** — frame loop, plugin host, built-ins as first-party plugins, `check`; no layout engine                                          | U3 U4 U8 U9 U12     | approved — 1 of 8 criteria fully met. Loop, plugin host, spinner, `flagstaff check`, all five built-ins, `flagstaff/ora` 99 / 99, `flagstaff/log-update` 99 / 99, `flagstaff/boxen` 84 / 84. **Not met:** the five conformance cases and the CR check name a demo that does not use flagstaff; the 20-run snapshot lives in flagstaff, not `burgee/testing`; ~~`/benchmarks` is 404~~ (the page is generated and published); ~~cli-table3 ungraded~~ (**29 / 29**); the U9 eval has never run |
...
| 27 | [`output-stack-compat/`](./output-stack-compat/) | **backwards compatibility** — eight façades graded by eight vendored suites; eight scoreboard rows | U11, C1–C6 | review — **5 of 8** graded (chalk 58 / 58, ora 99 / 99, log-update 99 / 99, boxen 84 / 84, **cli-table3 29 / 29**). **Decision taken 2026-09-08:** a drawing is a contract, so all four snapshot-graded hosts are built and gated; internal-module cases stay ungated (C4) and every row publishes three numbers — gated, internal, drawing. Eight rows is reachable |
...
Umbrella [`cli-foundation-stack/`](./cli-foundation-stack/) — the four layers _underneath_
...
| 31  | [`linegauge/`](./linegauge/)                       | **linegauge** — width · wrap · truncate · slice · strip · widest over `Intl.Segmenter`; 12 incumbents, 2.16 B/wk                                                                                  | Y2 Y3 Y7 Y8 Y11 Y12    | approved                                                                                                                                                                                                                                                                                                                                                  |
| 35  | [`paratext/`](./paratext/)                         | **paratext** — OSC: hyperlinks, images, title, clipboard, notifications, cwd, bell; every capability a data record with a static fallback; `register()` so a new terminal is a plugin, not a fork | Y1 Y3 Y5 Y7 Y8 Y9      | approved (D-129) — **written after the package shipped** (0.2.0, 5 files, 40 tests, no intent until 2026-09-13). R1–R7 built. **Not met:** R8 `ansi-escapes` default export, R9 three suites unvendored while the npm description says "drop-in", R10 a third plugin schema (`409dbeb2…` vs the family's `2f1bd1c6…`), R11 no ceiling, R12 no same-repo consumer |
| 32  | [`closeout/`](./closeout/)                         | **closeout** — handlers that run exactly once on every exit path, **with a deadline**; terminal restore; 6 incumbents, 685 M/wk                                                                   | Y3 Y5 Y6 Y7 Y8 Y10 Y12 | approved                                                                                                                                                                                                                                                                                                                                                  |
| 33  | [`seniority/`](./seniority/)                       | **seniority** — precedence as declared data, discovery, and **provenance**; 16 incumbents, 1.81 B/wk                                                                                              | Y2 Y3 Y5–Y8 Y10 Y12    | approved                                                                                                                                                                                                                                                                                                                                                  |
| 34  | [`bellpull/`](./bellpull/)                         | **bellpull** — executable resolution and a subprocess _result_ with a static projection; 15 incumbents, 2.29 B/wk; **carries a kill gate on `tinyexec`**                                          | Y3 Y5–Y8 Y10 Y12       | approved                                                                                                                                                                                                                                                                                                                                                  |
| 35  | [`agent-surface-declared/`](./agent-surface-declared/) | **N6** — a command's agent surface is declared, never forgotten. `--mcp` refuses to serve a program where a runnable command says nothing, naming every silent one at once; `effects: 'none'` is how an author declines on purpose | N2 N6                  | approved                                                                                                                                                                                                                                                                                                                                               |
| 36  | [`vendor-resolves-its-own-tree/`](./vendor-resolves-its-own-tree/) | **R6** — a vendor run reads the tree it writes to. A bare specifier walked out of a worktree into the parent checkout and graded one tree's code against another's host registry; it bit two lane agents in one session | C4 R6                  | approved                                                                                                                                                                                                                                                                                                                                               |
| 37  | [`schema-is-validated/`](./schema-is-validated/) | **F1** — `--schema` output validates against a schema this package publishes. `schemaVersion: 1` ships; the JSON Schema does not exist, and `JSON-Schema validated` sat on two READMEs until it was struck | F1 F2                  | approved                                                                                                                                                                                                                                                                                                                                                 |
| 38  | [`u5-weight-claim/`](./u5-weight-claim/)       | **U5** — a published claim that is false. `burgee/yargs ÷ yargs` is 1.038 against "no heavier in a user's bundle than the package it replaces"; the only measured lever is a product decision | U5                     | review                                                                                                                                                                                                                                                                                                                                                 |
| 39  | [`burgee-migrate/`](./burgee-migrate/) | **A1–A9** — `burgee migrate` takes a commander or yargs project, rewrites every import to the drop-in front-end and reports what changed: files, imports, dependencies removable, and the graded pass rate for each host it touched. Every migration that actually happened carried the codemod first — `jest-codemods`, `pnpm import`, `biome migrate eslint` — and `examples/demo-cli-commander` already holds the same program written both ways, which is the gate | A1 A9                  | draft                                                                                                                                                                                                                                                                                                                                                  |
| 40 | [`facade-gets-more/`](./facade-gets-more/) | **G1–G6** — what a migrated user gets for changing one import and nothing else. `tools/list` was `{"tools":[]}` for every commander and yargs program, and `tools/call` on one that **had** declared its effects returned no reply at all; `--json` was `unknown option` at the root of a command group and swallowed the operand after it. An undeclared command is now a tool that says `effects: "undeclared"`, `withheld` still means absent, and a façade failure under `--json` is the envelope with `fix`. commander 1360 / 1360 and yargs 804 / 804 unmoved | G1–G6, N1 N2 E3 | review |
...
dependencies with it; the pitch inverts from _install one more_ to _install one, remove the
dozen it stands in for_. **Two of the four are written either way** — `flagstaff` has `src/width.ts`
...
purpose — _one repo per family, so an agent's context holds the whole stack_. Measured
...
| :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------ | :---------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~**Give `roundel`, `flagstaff` and `caique` a publish credential.**~~                                                                                                     | npm trusted publisher           | done                                                                    | **closed 2026-09-09** — `roundel@0.1.0`, `flagstaff@0.1.0` and `caique@0.1.0` published at 18:14Z, and `release.yml` has run green five times since. This row sat open in the roadmap for a day after it was fixed, which is the reason for the audit above |
...
compat-commander    ████████████████████████  1360 / 1360  100.0%     ████████████████████████  1360 / 1360  100.0%
...
Every file of every suite is vendored and run — nothing is excluded. The _internals_ lines
...
| :-------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0                     | `sdlc-locks-evals-bands`, `cli-testing-harness`                                                                                                                    | the loop, and a harness that runs a CLI in-process                                                                                                                                                                                                                 | 🔨 **not shipped, corrected 2026-09-09** — the locks and the harness are real and green; the _bands_ half never ran (`control-bands.yml` has zero runs; 3 of 5 bands have no collector), and the harness's ANSI-regression check does not exist. Both intents moved back to `review`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **1 · engine**        | `replacement-parser`, `compat-oracle`, `cli-packaging`                                                                                                             | a one-file CLI that runs, the shape lock green, the first published pass rate                                                                                                                                                                                      | 🔨 engine built · oracle grading six hosts with controls · pass rate published on the docs site. **Left:** C3 is Node 24 only. ~~no compat control band~~ (six compute) and ~~three of four packages cannot publish~~ (all four have)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **3 · surfaces**      | `cli-mcp`, `commander-schema`, `commander-env`, `commander-completions`                                                                                            | `--schema`, `--mcp`, completions — the reason to switch                                                                                                                                                                                                            | 🔨 `--schema`, `--mcp` and static completions for bash/zsh/fish/pwsh + Fig served from the manifest on every program, commander syntax included (N1–N6, N8, N9, D2–D5); one precedence order with `--explain`, `meta.provenance`, config discovery with `extends`, `--version` from the owning package.json (V1–V7); options declared once — inferred types, numbers, `multiple`, choices enforced, relations, Standard Schema, kebab on the CLI (S1–S8); `changed`, the action-required envelope, agent detection, the schema budget (N7, N11–N13) — 🔨 **not complete, corrected 2026-09-09.** The surfaces are built and demonstrable; their intents are not met. Three of the four gaps named here closed the same day: ~~`--no-` completions do not exist~~ (#160, #162 — the negations did not exist either, and every boolean is now negatable because the precedence order requires it); ~~nothing validates the Fig spec~~ (#165 — the spec and the parser are checked to describe the same program; _not_ schema conformance, which needs `@withfig/autocomplete-types`); ~~`--version` before a command exits 2~~ (#158 — both incumbents print and exit 0). **Still open:** `--schema` emits `schemaVersion: 1` and drops `relations`; issue coverage is 11 of ~23 (S) and 8 of ~24 (V); there is still no "fails on plain commander" assertion anywhere, which is the one that matters most — a surface nobody has shown the incumbent lacks is not yet a reason to switch |
...
At one calendar month per wave, F1–F4 is four months running _under_ S1–S4 rather than after
...
needs width _now_, so the code is written in F1's window either way; `linegauge` is the only
...
| A foundation layer is absorbed by the platform                  | a Node builtin becomes a _substitute_ rather than a component at any LTS                                 | the package is deprecated on purpose (Y4), with a README that says so. Two candidates already died to this test before publishing; a published name gets the same discipline                                                                                                                                 |
...
of a _user's_ CLI (language-free, removes the 30 ms and the Node install) plus the lint
...
5. **The largest speed win is language-independent and comes first:** single-binary
   distribution of a _user's_ CLI — `burgee build --binary` via Node's single-executable
...
7. **What the people who did it concluded** — reviewed with sources in
   ...
   sharpenings follow: measure the addon's _load_ cost against the work saved; native lives
...
> (with X8) and as _"next"_ in the same cell. That is what a status column becomes when it
...
- **`cli-help-renderer` promoted to wave 2.** Help rendering is the largest cluster in the
  trackers _and_ the single most-requested open item in either project — yargs #684,
...
- **`cli-benchmarks` B1 is now a hypothesis, not a target.** JetBrains' 425-trial study
  found output filtering _raised_ cost 7.6%. O2/O5/F1 are justified by parse reliability.
...
| `node-matrix-green`           | every Node LTS in `engines` × Linux, macOS, Windows | **Node 24 only** on three OSes (2026-09-08). A deliberate narrowing while the output stack lands, so C3 is _not_ met: `engines` says `>=24` and 26 is not exercised. Restored by adding 26 back to the matrix in `compat.yml` — one line, and nothing else changes. **Restored 2026-09-23:** 26 is back in the matrix after the whole suite passed on v26.10.0. |
...
| :----------------------------------- | ----: | ---: |
| military rank, one above _commander_ |    20 |    0 |
| command words                        |    22 |    2 |
...
careless typist. It is not a plausible _misspelling_ the way `invokable` is — they are
...
_"write programs to handle text streams, because that is a universal interface"_, the
...
| :-------------------------------------------------------------------------------------------- | :------------------------ | :------------------------------------------------------ |
| the framework                                                                                 | `burgee`                  | ✅ `burgee@0.11.1`                                       |
...
| the colours a CLI carries: policy, tokens, theme, chalk path                                  | `roundel`                 | ✅ `roundel@0.5.1` — chalk path graded 58 / 58          |
| the staff the flag flies from: frame loop, plugin host, ora/boxen/cli-table3/log-update paths | `flagstaff`               | ✅ `flagstaff@0.4.1`                                    |
| the parrot that always answers back: prompts, flags first                                     | `caique`                  | ✅ `caique@0.5.1`                                       |
| measuring and cutting styled text: width, wrap, truncate, slice                               | `linegauge`               | `linegauge@0.5.1` stub; F1                              |
| ending cleanly: exactly-once handlers, a deadline, terminal restore                           | `closeout`                | `closeout@0.5.1` stub; F2                               |
| which source outranks which, and which one set this value                                     | `seniority`               | `seniority@0.5.1` stub; F3                              |
| running another program and getting a result back                                             | `bellpull`                | `bellpull@0.3.1` stub; F4, conditional on its kill gate |
...
_artifact outside the code_ (a benchmark, a band, a page, a published tarball, an adopter) is
unmet.

### 1. The benchmark suite: three axes of four, and B1 still unmeasured

_Updated 2026-09-09._ **Three of the four axes landed.** `benchmarks/` exists, `npm run bench`
...
not exist and reporting _"band not computed yet"_ forever. Both still stand at **0 of 8**,
deliberately.
...
- **Three of five bands have no collector that can run.** _Fixed 2026-09-09 for two of the
  three._ `npm run bench` now writes `benchmarks/results/<suite>/<date>.json`, `bench.yml`
  ...
  bands remain at 0 of 8 — their axis needs a credential — but they now say _why_, in the
...
- **The evals link checker does not read `.sdlc/intents/`.** `scripts/run-evals.ts` scans 14
  agent docs and none of them is under this directory — so a broken link in _this file_, the
...
- **The post-deploy SHA gate** (`docs-deploy`) — it _does_ fail, on good deploys: run
...
| `cli-help-renderer` #3                     | the two front-ends render byte-identical help to each other | the shipped contract is the **opposite**: each is byte-identical to _its own incumbent_, and the two incumbents differ   |
...
| `output-stack-compat` #1                   | eight rows including clack and inquirer                     | a 2026-09-08 decision records both as blocked — a façade matching their drawing snapshots would _be_ the incumbent       |
...
- **`plugin-contract`** — one of four, and that one only at the two packages that already comply:
  ...
  are found _not applicable_ rather than failing.
