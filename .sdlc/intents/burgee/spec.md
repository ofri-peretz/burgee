...
| E7 | **Built** 2026-09-24 | Both halves this row called absent shipped in #472 (D-118) and the row was not moved. **The author half:** `define-error.ts`'s `defineError({ name, code })` returns a class, and `execute.ts`'s `describeFailure` reads its code off `Symbol.for('burgee.exitCode')` on the thrown instance's class — a subclass inherits it — so the program leaves with that code, rendered like `UsageError` (`hint`, `fix`, and the `--json` envelope's `error.code`). **The startup half:** the class registers its code when `defineError` runs, which is when the module declaring it loads — before any program code. A second class claiming a code throws there, and so does a claim on the contract's own codes or on 126 and up. Checked across modules on 2026-09-24: two files each defining code 10, imported by a third, exit 1 with *which "QuotaError" already owns* before the importer's first line runs, because both import the one `define-error.js`. The mapping half has been a table (`CLASSIFIED`) since 2026-09-22, and the build-time locks on the contract's own codes stand beside it | `define-error.test.ts`: *"leaves with the declared code and renders the message, hint and fix"*, *"refuses a second class claiming a code, when it is defined"*, *"refuses the contract’s own codes and anything a shell reads as something else"*; `exit-code.test.ts`, `scripts/exit-code-lock.test.ts` |
...
| J3 | **Built** | `.burgee({ floor: true })` on both façades (D-121). Off by default, so the hosts' suites still grade 1360 / 1360 and 804 / 804. With it, and nothing else injected: a usage failure exits 2 (E1) rather than the host's 1; an action or handler that throws or rejects is one line and its E1 code — no stack, and on yargs no help screen (no-help-on-runtime-error); `--help` still exits 0. It injects nothing else: a returned value is not printed, and `--json` still settles a failure to the envelope (D-140). A commander program that called `exitOverride()` keeps its exits. Every case is paired with the same program without the call, which keeps the host's behaviour. No module is added: the floor is three checks routing into code D-140 already put on the façades' startup path, and the core bundle is unchanged (30,107 B) | `facade-floor.test.ts`, a real child process per case |
| J4 | **Built** | *"the program wins"* was built; *"reported by `--schema`"* now is: `ProgramSchema.shadows` names the reserved surfaces a façade program declares for itself and burgee therefore withholds — `--json`, `--mcp`, `completion` (yargs' own `.completion()` under any name included) — and is absent when it shadows none. `burgee/program-schema.json` describes it, and each façade's document is validated against the file. `--schema` itself cannot be reported: a program that declares it answers it. A native program cannot shadow them — `defineProgram` refuses the names (V5) | `scripts/program-schema.test.ts`: *"--schema names the reserved surfaces a façade program shadows (J4)"*; `adoption-ladder.test.ts`: *"leaves a program that declares its own `--schema` option alone"* |
...
| # | Status | Evidence | The check |
| :-- | :-- | :-- | :-- |
| B1 | **Built** | measured for the first time on 2026-09-24 (`2a51440`, D-147): 5 tasks × 5 runs × 2 builds on claude-sonnet-4-5, the owner's subscription login, `~/.claude` isolated. Per task, burgee median **78,225 tokens / 3 turns / 0.80 success**; commander **130,165 / 5 / 0.84**. Ratios **tokens 0.601, turns 0.600**: `agent-turns-30pct` **met**, `agent-tokens-40pct` **NOT met** — 0.601 against ≤ 0.6, a miss of 0.001 on one run of n = 25 a side, so the honest reading is "about 40%, not shown to be at least 40%". burgee's success rate sits exactly on the 0.8 floor. One local observation: the bands read CI runs only (D-142), so the weekly job still owes the series | `benchmarks/results/agent-cli-bench/2026-09-24-2a51440-local.json`; `benchmarks/agent.test.ts` drives the harness against a stub `claude`; `emit.test.ts` proves an unmeasured axis cannot read as measured |
...
| N10 | **Built** 2026-09-24 | Both tools exist, run offline, and are **not on npm** — pinned by version and archive SHA-256 in `benchmarks/floor-tools.ts` (clispec-cli 0.3.0, cli-agent-lint 0.3.5; the npm `clispec` is an unrelated package). `benchmarks/axes/floor.ts` grades the burgee, commander and yargs demos with both; the `floor` job in `bench.yml` runs it on every PR; the first CI run is published as `benchmarks/results/external-floor/2026-09-24.json` and on `/docs/benchmarks` (D-149). **The measurement is built; the floor does not pass it.** burgee scores **6 / 24** on clispec (Needs Work; commander and yargs 2) and **78%** on cli-agent-lint (B; commander 71.2, yargs 70.9). 15 of clispec's 18 failures follow from one choice — clispec probes a `schema` subcommand and burgee answers `--schema`: ten test that command directly, and the idempotent and bounded principles (five checks) fail outright without it (`src/checks/{idempotent,bounded}.rs` return early). The other three are the output principle: the piped-output declaration, the error envelope as the last line of stderr, and an explicit format winning over TTY detection. All of it is GAPS B22, not argued away | `benchmarks/floor.test.ts`; burgee's two scores gated at 6 and 78 by `npm run bench -- --axis floor --check` |
...
| U5 | **Built** | Restated by D-148, with the residual measured. Every entry point's non-default paths sit behind `await import()` — failure (`failure.js`), relations (`relations.js`), help, `--version`, completions, `config explain`, `--schema` and `--mcp` (`surfaces.js`), config layers (`config-layers.js`) — and its bundled bytes are a ratchet at the measurement. Measured 2026-09-24: `burgee/yargs ÷ yargs` **0.961**, met; `burgee ÷ cac` **2.316** (30,178 → 24,202 B), whose 13,750 B above cac are the default run path, V1 precedence, E5 shutdown and `Manifest.use()`'s synchronous refusal; `burgee/commander ÷ commander` **1.561**, whose 21,930 B are pinned by `parse()` being synchronous (`--schema`, the `--json` envelope, the manifest) and by `bellpull/cross-spawn` (D-102). The family's other subpaths hold — chalk 0.574, picocolors 0.162, ora 0.933, boxen 0.35, cli-table3 0.692, log-update 0.504 | `npm run bench -- --axis weight --check`; `packages/burgee/src/weight.test.ts` |
...
| U9 | Not built | the design itself states the condition — *"locks when the weekly one-turn authoring eval reaches the pass rate it states"* (proposed 9 of 10, three consecutive weeks). **It has now run once, and passed 1 of 9** (2026-09-24, `2a51440`, claude-opus-5 on the owner's subscription login, D-147; `evals/history/2026-09-24-2a51440.json`). The eight failures are not wrong plugins: each ended `error_max_turns` with no file written — the evals' default `EVAL_MAX_TURNS=3` is fewer turns than the prompt's own steps (read the schema, read the README, write, run `check`), and a re-run of the flagstaff case read `terminal_reason: max_turns`. So the eval as configured cannot measure the claim; one week of three, and that week missed. The schema and `llms.txt` halves exist | `evals/history/2026-09-24-2a51440.json` |
...
| U12 | Not built | the design states the condition — *"locks when the independence install test passes for every layer and the first adopter installs a layer alone"*. **The first half is built and passing (2026-09-24):** for each of the nine published packages, `scripts/independence-install-lock.test.ts` packs the tree, installs that tarball alone — plus the tarballs of the in-family packages it declares, `--offline`, `--no-package-lock` — into an empty directory, imports every entry in its `exports` map under Node, and records every resolution with a `module.registerHooks` hook: each cross-package edge must land on a package the importer declares, so a sibling present only because another package hoisted it fails too. Proven red by removing `roundel` from burgee's `dependencies` (`ERR_MODULE_NOT_FOUND` from `burgee/contrast`) and by a `linegauge` that imports `paratext` (flagstaff's install still loads; the edge check names it). **What remains is the second half, and it is not ours to build:** a CLI we did not write installing one layer alone (GAPS C3) | `scripts/independence-install-lock.test.ts`: *"installs nothing but itself and the in-family packages it declares"*, *"loads every module entry in its `exports` map"*, *"reaches no package it does not declare"* |
