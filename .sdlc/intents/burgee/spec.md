...
| E7 | **Built** | `defineError({ name, code })` returns an `Error` class whose throw leaves with `code`, rendered like `UsageError` — message, `hint`, `fix`, and the JSON envelope's `error.code` — and a subclass inherits it. A second class claiming a code throws when it is defined, which is when its module loads; codes are 7–125, so the contract's seven and a shell's 126+ cannot be claimed (D-118). The engine reads the code off `Symbol.for('burgee.exitCode')` and never imports the module. The built-in half is unchanged: `CLASSIFIED` in `execute.ts`, with the no-reuse property locked at build time by `exit-code.test.ts` and `scripts/exit-code-lock.test.ts` | `define-error.test.ts`, `exit-code.test.ts`, `scripts/exit-code-lock.test.ts` |
