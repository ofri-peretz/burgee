...
| Claim | Gate | Measured | |
| :--- | :--- | ---: | :--- |
| the core entry point is under 52 KB bundled | `core-under-52kb-bundled` | 24,551 bytes | ✅ met |
| `burgee/yargs` is lighter in a user's bundle than `yargs` | `lighter-than-yargs` | 0.958× | ✅ met |
| `burgee` is lighter than `cac` **plus what a cac user installs to match it** | `lighter-than-cac-at-parity` | 0.251× | ✅ met |
...
| `burgee` is lighter in a user's bundle than `cac` alone | `lighter-than-cac` | 2.349× | ❌ **not met** |
...
`burgee` is 24,551 bundled bytes and `cac` is 10,452, so the bare row reads **2.349× and it
...
| :--- | ---: | ---: | ---: |
| `cac` | 10,452 B | **97,711 B** | 24,551 B |
...
parser and help renderer; burgee's 24,551 is that plus coercion, choices, relations, Standard
