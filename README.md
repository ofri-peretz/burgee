...
| Claim | Gate | Measured | |
| :--- | :--- | ---: | :--- |
| the core entry point is under 52 KB bundled | `core-under-52kb-bundled` | 27,996 bytes | ✅ met |
| `burgee/yargs` is lighter in a user's bundle than `yargs` | `lighter-than-yargs` | 0.958× | ✅ met |
| `burgee` is lighter than `cac` **plus what a cac user installs to match it** | `lighter-than-cac-at-parity` | 0.287× | ✅ met |
...
| `burgee` is lighter in a user's bundle than `cac` alone | `lighter-than-cac` | 2.679× | ❌ **not met** |
...
`burgee` is 27,996 bundled bytes and `cac` is 10,452, so the bare row reads **2.679× and it
...
| :--- | ---: | ---: | ---: |
| `cac` | 10,452 B | **97,711 B** | 27,996 B |
...
parser and help renderer; burgee's 27,996 is that plus coercion, choices, relations, Standard
