 ...
 * name — they "keep their own copy, because burgee has to install alone" — and the copy
 * is what that bought: `precedence.ts` and `config.ts`, 281 lines implementing the whole
 * of seniority's stated job, inside burgee, where the rule guaranteed they could never
 * move. A layer nobody above may use is not a layer; it is a suggestion.
 *
 * The arrow still points one way, and that is the part worth enforcing: the foundation
 * depends on nothing, so no edge can come back up. Installing burgee now installs
 * seniority with it — one more package on disk, against one copy of the logic, one test
 * suite, and one place a fix lands.
 */
...
function checkCodeExports(exports: Record<string, unknown>): void {
  const entries = Object.entries(exports).filter(
    ([k]) => k !== "./package.json" && !isDataExport(k),
  );
  expect(entries.length).toBeGreaterThan(0);
  for (const [entry, target] of entries) {
    expect(typeof target, `${entry} must be a conditions object`).toBe(
      "object",
    );
    const conditions = target as Record<string, string>;
    expect(Object.keys(conditions).sort(), entry).toEqual([
      "default",
      "import",
      "types",
    ]);
    expect(
      conditions["default"],
      `${entry}: default must be the same ESM file as import`,
    ).toBe(conditions["import"]);
  }
}
...
function checkDataExports(exports: Record<string, unknown>): void {
  for (const [entry, target] of Object.entries(exports).filter(([k]) =>
    isDataExport(k),
  )) {
    expect(
      typeof target,
      `${entry} is data; it takes a path, not a conditions object`,
    ).toBe("string");
...
describe.each(published)("published package $pkg.name", ({ dir, pkg }) => {
  it("has no external runtime dependencies, and same-repo ones only point up the family (K1, U1, U6)", () => {
    ...
    const offenders = Object.keys(pkg.dependencies ?? {}).filter((dep) => {
      // The foundation is below everything, so an edge into it is always downward.
    ...
    expect(
      offenders,
      `${pkg.name} may depend only on earlier family packages, or on the foundation tier`,
    ).toEqual([]);
  });
  ...
  it("imports none of the packages Node ships natively (K3)", () => {
    ...
    for (const file of readdirSync(src).filter(
      ...
      for (const name of BANNED) {
        if (new RegExp(`from ['"]${name}['"]`).test(text))
...
describe("the foundation tier (Y1)", () => {
  ...
  it("is not empty — otherwise every assertion below passes by having nothing to check", () => {
    expect(
      FOUNDATION.filter((n) => manifestOf(n) !== undefined).length,
    ).toBeGreaterThan(0);
  });
  ...
  /*
   * The half of Y1 that survives its reversal, and the only half that was ever load-
   * bearing: the tier is a floor. A foundation package reaching back up to the family
   * would make the two mutually dependent and the "install it on its own" claim false.
   */
  it.each(FOUNDATION)(
    "%s reaches nothing in the family: the arrow points one way",
    (name) => {
      const manifest = manifestOf(name);
      if (manifest === undefined) return; // a reserved name with no package yet is not a finding
      const upward = Object.keys(manifest.dependencies ?? {}).filter((d) =>
        FAMILY_ORDER.includes(d),
      );
      expect(
        upward,
        `${name} is the floor; nothing it depends on may sit above it`,
      ).toEqual([]);
    },
  );
});
