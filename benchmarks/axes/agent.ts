 ...
 * against a canned response, `runOne` is driven end to end against a stub `claude` binary
 * (`agent.test.ts`), and every task's `check` is proved to fail on the un-run state
 * (`tasks.test.ts`). What is untested is the model's behaviour, which is the thing the
 * credential buys.
 */
...
export function blockers(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  if (spawnSync('claude', ['--version'], { stdio: 'ignore' }).status !== 0) reasons.push('the `claude` binary is not on PATH');
  for (const v of VARIANTS) {
    if (!existsSync(join(REPO_ROOT, v.bin))) reasons.push(`${v.bin} is not built`);
  }
  return reasons;
}
...
function variantRecords(variant: VariantId, attempts: Attempt[], model: string): BenchRecord[] {
  ...
  return [
    { ...common, metric: 'tokens-per-task', unit: 'tokens', median: median(tokens), p95: median(tokens), note: `median over ${String(attempts.length)} task-runs on model ${model}` },
    { ...common, metric: 'turns-per-task', unit: 'turns', median: median(turns), p95: median(turns), note: "`num_turns` as `claude -p --output-format json` reports it" },
    {
      ...common,
      metric: 'success-rate',
      unit: 'ratio',
      median: round(successes / attempts.length, RATIO_PLACES),
      p95: round(successes / attempts.length, RATIO_PLACES),
...
export function run(options: AgentOptions = {}): { records: BenchRecord[] } | { reason: string } {
  ...
  for (const variant of VARIANTS) {
    const toolDir = installTool(join(REPO_ROOT, variant.bin));
    const attempts: Attempt[] = [];
    for (const task of tasks) {
      for (let i = 0; i < runs; i++) {
        const workdir = mkdtempSync(join(tmpdir(), `bench-${task.id}-`));
        mkdirSync(workdir, { recursive: true });
        attempts.push(runOne({ claudeBin: options.claudeBin ?? 'claude', task, toolDir, workdir, model, timeoutMs: options.timeoutMs ?? DEFAULT_TIMEOUT_MS }));
      }
    }
